...
public class RecipeControllerTest {
	...
	public void init() throws Exception {
		...
		this.mock = MockMvcBuilders.standaloneSetup(recipeController)
	...
	public void testGetNewRecipe() throws Exception {
		this.mock.perform(get("/recipe/new")).andExpect(status().isOk()).andExpect(model().attributeExists("recipe"))
				.andExpect(model().attribute("recipe.id", nullValue())).andExpect(view().name("recipe/recipeform"));
	}
	...
	public void testPostNewRecipeForm() throws Exception {
		...
		mock.perform(post("/recipe")
				.contentType(MediaType.APPLICATION_FORM_URLENCODED)
					.param("id", "")
					.param("description", sDescription).param("directions", "new directions"))
